...
v99 solve()
{
    /// code is here->
    v99 n;cin>>n;
    map<v99,v99>mpp;v99 a[n+2];
    rep0(i,0,n)cin>>a[i],fre[a[i]]++;
    v99 q;cin>>q;
    while(q--)
    {
        char c;v99 x;
        ...
        rep1(x,1,100000)
        {
            ...
            if(fre[x]>5)cnt[6]++;
            if(fre[x]>7)cnt[8]++;
            if(cnt[8] || cnt[4]>1 || (cnt[6] && cnt[2]>1)|| (cnt[4] && cnt[2]>2))break;
        }
        if(cnt[8] || cnt[4]>1 || (cnt[6] && cnt[2]>1)|| (cnt[4] && cnt[2]>2))cout<<"YES"<<ses;
        else cout<<"NO"<<ses;
        //cout<<cnt[2]<<whp<<cnt[4]<<whp<<cnt[6]<<whp<<cnt[8]<<ses;
        cnt[2]=cnt[4]=cnt[6]=cnt[8]=0;
    }
}
