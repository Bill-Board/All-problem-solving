...
///-------------------------------------------------------------------------------------------------------
v99 n,m,p[mxi],h[mxi],visi[mxi],val[mxi],balo[mxi];
vector<v99>a[mxi];
bool ok=false;
v99 dfs(v99 ind)
{
    ...
    val[ind]=s+p[ind];
    balo[ind]=(val[ind]+h[ind])/2;
    if((val[ind]+h[ind])&1)ok=true;
    if(2*balo[ind]-val[ind]!=h[ind])ok=true;
    if(balo[ind]<0 || balo[ind]>val[ind])ok=true;
    if(b>balo[ind])ok=true;
    //cout<<val[ind]<<whp<<ind<<whp<<godmd<<whp<<ok<<ses;
    return val[ind];
}
