...
{
    /// code is here->
    v99 n,m,am=0,an=LLONG_MAX,bn=LLONG_MAX;cin>>n>>m;
    rep0(i,0,n){v99 x;cin>>x;am=max(am,x);an=min(an,x);}
    rep0(i,0,m){v99 x;cin>>x;bn=min(bn,x);}
    if(max(2*an,am)<bn)cout<<max(2*an,am)<<ses;
    else cout<<-1<<ses;
