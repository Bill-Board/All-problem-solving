...
void solve()
{
    /// code is here->
    v99 ans=0;b.clear();
    for(v99 i=0;i<n;i++)scanf("%lld",&a[i]);
    for(v99 i=0;i<n;)
    {
        if(b.empty() or a[b.back()]<=a[i])b.pb(i++);
        else
        {
            v99 kot=b.back();b.pop_back();
            ans=max(ans,a[kot]*(b.empty()?i:i-b.back()-1));
        }
    }
    while(!b.empty())
    {
        v99 kot=b.back();b.pop_back();
        ans=max(ans,a[kot]*(b.empty()?n:n-b.back()-1));
    }
    printf("%lld\n", ans );
}
