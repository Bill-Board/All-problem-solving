...
v99 kortesi()
{
    v99 lipmas[]={0,31,29,31,30,31,30,31,31,30,31,30,31,0};
    v99 mas[]={0,31,28,31,30,31,30,31,31,30,31,30,31,0};
    v99 cr_h=0,mst=a[0].m,dinst=a[0].d;
    for(v99 i=a[0].y;i<=a[1].y;i++)
    {
        for(v99 j=mst;j<13;j++)
        {
            v99 kot;
            if(i%400==0)kot=lipmas[j];
            else if(i%100==0)kot=mas[j];
            else if(i%4)kot=mas[j];
            else kot=lipmas[j];
            for(v99 k=dinst;k<=kot;k++)
            {
                cr_h++;
                if(i==a[1].y && j==a[1].m && k==a[1].d)
                    return cr_h-1;
                //cout<<i<<whp<<j<<whp<<k<<ses;
            }
            dinst=1;
        }
        mst=1;
...
{
    /// code is here->
    v99 y1,y2,m1,m2,d1,d2;
    char c;
    cin>>y1>>c>>m1>>c>>d1;a.push_back({y1,m1,d1});
    cin>>y2>>c>>m2>>c>>d2;a.push_back({y2,m2,d2});
    sort(sob(a),ssss);
    cout<<kortesi()<<ses;
}
