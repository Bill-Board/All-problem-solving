...
{
    /// code is here->
    v99 n,k;cin>>n>>k;v99 conk[n+2],blk[n+1];
    string s="1";
    rep0(i,0,n)
    {
        char c;cin>>c;s+=c;
    ...
    rep1(i,1,n)
    {
        conk[i]+=(s[i]==':');conk[i]+=conk[i-1];
        blk[i]+=(s[i]=='X');blk[i]+=blk[i-1];
        if(s[i]=='I')f.push(i);
        if(s[i]=='M')m.push(i);
    }
    ...
    while(!f.empty())
    {
        ...
        {
            v99 dif=k+1-(conk[u]-conk[r])-(u-r);
            if(dif>0)ok++,f.pop(),m.pop();
            else f.pop();
        }
        else
        {
            v99 dif=k+1-(conk[r]-conk[u])-(r-u);
            if(dif>0)ok++,f.pop(),m.pop();
            else m.pop();
        }
    }cout<<ok<<ses;
