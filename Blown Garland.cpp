...
int main()
{
    ...
    //while(1)
    {
        string s;cin>>s;
        string k="";v99 i=0;
        while(s[i]=='!' && i<s.size())i+=4;k+=s[i];i=1;
        while(s[i]=='!' && i<s.size())i+=4;k+=s[i];i=2;
        while(s[i]=='!' && i<s.size())i+=4;k+=s[i];i=3;
        while(s[i]=='!' && i<s.size())i+=4;k+=s[i];
        map<char,v99>mpp;
        rep0(i,0,s.size())
        {
            if(s[i]!=k[i%4])mpp[k[i%4]]++;
        }
        cout<<mpp['R']<<whp<<mpp['B']<<whp<<mpp['Y']<<whp<<mpp['G']<<ses;
