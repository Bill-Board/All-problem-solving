...
{
    /// code is here->
    v99 n,cr_h=0;string s;cin>>n>>s;
    CLR(visi,0);
    rep0(i,0,n)
    {
        v99 x=(i+1)%n;
        if(s[i]=='R' && s[x]=='L' && visi[i]==0 && visi[x]==0)
        {
            visi[i]=1;
            visi[x]=1;
        }
    }
    //rep0(i,0,n)cout<<s[i]<<whp;cout<<ses;
    //rep0(i,0,n)cout<<visi[i]<<whp;cout<<ses;
    rep0(i,0,n)
    {
        v99 sam=(i+1)%n,piche=i-1;
        if(i==0)piche=n-1;
        if(visi[i])continue;
        if(s[i]=='L' && s[piche]=='L' && visi[piche])continue;
        if(s[i]=='R' && s[sam]=='R' && visi[sam])continue;
        if(s[i]=='L' && s[piche]=='L')s[piche]='R',visi[i]=1,visi[piche]=1;
        else if(s[i]=='R' && s[sam]=='R')s[sam]='L',visi[i]=1,visi[sam]=1;
        //cout<<i<<whp<<visi[i]<<ses;
        cr_h++;
        //rep0(j,0,n)cout<<visi[j]<<whp;cout<<ses;
    }
    cout<<cr_h<<ses;
