...
void kot()
{
    for(v99 i=1;i<=5000;i++)
    {
        map<v99,v99>mp;
        for(v99 j=i;j<=10000;j+=i)
            mpp[j][i]++,q++;
    }
    for(v99 i=1;i<=10000;i++)q+=sqrt(i);
    for(v99 i=1;i<=100;i++)q+=sqrt(i);
}
...
void solve()
{
    ...
    rep1(i,1,n)
    {
        ...
        for(v99 j=1;j<=q;j++)
        {
            if(i%j==0 and (j==x || i/j==x)){ok=true;break;}
        }

        if(!ok){cout<<"NO"<<ses;return;}
    }
    cout<<"YES"<<ses;
}
