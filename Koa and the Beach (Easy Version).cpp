...
void solve()
{
    v99 n,k,l;cin>>n>>k>>l;v99 a[n+2];
    rep0(i,0,n)cin>>a[i];vector<v99>p;
    rep1(i,0,k)p.pb(i);
    for(v99 i=k-1;i>0;i--)p.pb(i);
    v99 t=1;
    for(auto i:p)cout<<i<<whp;cout<<ses<<ses;
    for(v99 i=0;i<n;)
    {
        v99 x=a[i];
        rep0(j,i,n){v99 kot=x+p[t%(2*k)];a[j]=kot;}
        if(a[i]<=l)i++;
        t++;cout<<a[i]<<whp<<t<<ses;
    }
    cout<<t<<ses;
}
