...
int main()
{
    ...
    //while(1)
    {
        v99 n,apnt=0,bpnt=0;cin>>n;
        while(n--)
        {
            v99 x,y;cin>>x>>y;
            v99 kota=num(x),kotb=num(y);
            if(kota<kotb)bpnt++;
            else if(kota>kotb)apnt++;
            else apnt++,bpnt++;
        }
        if(apnt>bpnt)cout<<0<<whp<<apnt<<ses;
        else if(apnt<bpnt)cout<<1<<whp<<bpnt<<ses;
        else cout<<2<<whp<<bpnt<<ses;
    }
    return 0;
}
