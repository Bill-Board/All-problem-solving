...
v99 subarray(vector<v99>a)
{
    v99 sum=0, ekdm_crh=0;
    for(auto i:a)
    {
        sum=max(sum+i,(v99)0);
        //sum+=i;sum=max(sum,(v99)0);
        ekdm_crh=max(ekdm_crh,sum);
    }
    return ekdm_crh;
}
...
void solve()
{
    v99 n,cr_h=0;cin>>n;v99 a[n+1];
    rep0(i,0,n){cin>>a[i];if(i%2==0)cr_h+=a[i];}
    vector<v99>ek,dui;
    for(v99 i=0;i<n-1;i+=2)ek.pb(a[i+1]-a[i]);
    for(v99 i=1;i<n-1;i+=2)dui.pb(a[i]-a[i+1]);
    cr_h+=max({0LL,subarray(ek),subarray(dui)});
    cout<<cr_h<<ses;
}
