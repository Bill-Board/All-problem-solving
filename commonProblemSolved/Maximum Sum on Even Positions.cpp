...
void solve()
{
    ...
    if(n>1)
    {
        v99 dpp[n+2];dpp[0]=0;CLR(dpp,0);
        rep0(i,2,n)
        {
            if(i%2==0)dpp[i]=max(0LL,dpp[i-2]+a[i-1]-a[i]);
            else dpp[i]=max(0LL,dpp[i-2]+a[i]-a[i+1]);
        }v99 kot=0;
        rep0(i,0,n)kot=max(kot,dpp[i]),cout<<dpp[i]<<whp;cr_h+=kot;
        cout<<ses;
    }
    cout<<cr_h<<ses;
}
