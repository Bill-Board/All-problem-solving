...
void kortesitoh()
{
    v99 m,a[10],err[10]={},ans[10];
    vector<pii>vp;
    rep0(i,0,n)scanf("%d",&a[i]),err[i]=INF;
    scanf("%d",&m);
    rep0(i,0,m)
    {
        v99 x,y;
    ...
    {
        v99 sum[10]={},oma[10]={};
        rep0(i,0,n)sum[i+1]=sum[i]+a[i];
        sum[n+1]=INF;
        for(v99 x=0,y=0;x<m;x++)
        {
            while(y<=n && sum[y]<vp[x].ff)y++;
            v99 kot=abs(sum[y]-vp[x].ff);
            if(y!=1)kot=min(kot,abs(sum[y-1]-vp[x].ff));
            oma[vp[x].ss]+=kot;
        }
        rep0(i,0,10)
        {
            if(oma[i]>err[i])break;
            if(oma[i]<err[i])
            {
                rep0(j,0,10)
                {
...
void solve()
{
    while(scanf("%d",&n)==1 && n)kortesitoh();
}
signed main()
{
    //dua
