...
int maximalNetworkRank(int n, vector<vector<int>>& roads) {
        ...
        {
            tom[roads[i][0]].push_back(roads[i][1]);
            tom[roads[i][1]].push_back(roads[i][0]);
            a[roads[i][0]][roads[i][1]]=a[roads[i][1]][roads[i][0]]=1;
        }
        int cr_h=0;
        for(int i=0;i<n-1;i++)
        {
            for(int j=i+1;j<n;j++)
            {
                if(a[i][j])cr_h=max(cr_h,(int)(tom[i].size()+tom[j].size()-1));
                else cr_h=max(cr_h,(int)(tom[i].size()+tom[j].size()));
            }
        }
        return cr_h;
    }
...
void solve()
{
    ...
    cout<<maximalNetworkRank(n,roads)<<ses;
