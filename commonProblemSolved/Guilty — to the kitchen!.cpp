...
void solve()
{
    ...
    rep0(i,0,n)cin>>a[i];
    rep0(i,0,n)cin>>b[i];
    double l=0,r=v,mx=0, eps=0.0000001;
    while(r-l>eps)/// maximum x ber korar jonno
    {
        double mid=(l+r)/2,kot=0;
        bool ok=true;
        for(v99 i=0;i<n;i++)
        {
            double limit=a[i]*mid;
            if(limit>b[i]){ok=false;break;}
            kot+=limit;
        }
        if(ok and kot>=mx and kot<=v)mx=kot,l=mid;
        else r=mid;
    }
    cout<<fixed<<setprecision(10)<<mx<<ses;
