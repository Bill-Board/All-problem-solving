...
{
    /// code is here->
    SieveOfEratosthenes(1009);
    //for(auto i:prm)cout<<i<<whp;cout<<ses;
    v99 n;cin>>n;
    if(n<=2){cout<<-1<<ses;return;}
    if(prime[n])
    {
        cout<<n<<ses;
        rep0(i,0,n)cout<<i+1<<whp<<((i+1)%n)+1<<ses;
        return;
    }
    auto kot=upper_bound(prm.begin(),prm.end(),n);
    v99 same=*kot;
    v99 dif=same-n;
    cout<<same<<ses;
    rep0(i,0,n)cout<<i+1<<whp<<((i+1)%n)+1<<ses;
    for(v99 i=2;dif>0;i+=4,dif-=2)
    {
        cout<<i-1<<whp<<i+1<<ses;
        if(dif>1)cout<<i<<whp<<i+2<<ses;
    }
    return;
}
