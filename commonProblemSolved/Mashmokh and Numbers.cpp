...
v99 solve()
{
    v99 n,k;cin>>n>>k;vector<v99>a;
    if(n==1) {
		if(k==0) cout<<1<<endl;
		else cout<<-1<<endl;
		return 0;
	}
    v99 z=n/2;if(z>k)return cout<<-1<<ses,0;
    v99 koita=k-(z-1);a.pb(koita);a.pb(2*koita);
    v99 p=koita,q=koita*2;
    --z;
    for(v99 i=1;i<=1000000 && z;)
    {
        if(i==p || i==q || i+1==q || i+1==p)i++;
        else a.pb(i),a.pb(i+1),i+=2,z--;
    }
    if(n&1)a.pb(1000000000);
