...
int main()
{
    ...
    //while(1)
    {
        v99 n,m;cin>>n>>m;vector<pair<v99,v99> >vp;
        vector<v99>tom[1009];
        rep0(i,0,n){v99 x;cin>>x;vp.push_back(make_pair(x,i+1));}
        ...
        rep0(i,1,n+1)sort(tom[i].begin(),tom[i].end());
        //for(auto i:vp)cout<<i.first<<whp<<i.second<<ses;
        for(auto i:vp)
        {
            v99 pos=i.second,val=i.first;
            if(tom[pos].empty())continue;
            cr_h+=(tom[pos].size() * val);
            for(auto j:tom[pos])
            {
                auto it=lower_bound(tom[j].begin(),tom[j].end(),pos);
                tom[j].erase(it);
            }
            tom[pos].clear();
        }cout<<cr_h<<ses;
