...
int main()
{
    ...
    //while(1)
    {
        v99 n;string s;cin>>n>>s;
        vector<v99>ami[101];
        vector<v99>ke(1001,1);
        rep0(i,0,n)
        {
            v99 a,b;cin>>a>>b;
            if(s[i]=='1'){rep0(j,0,1000)ke[j]=0;}
            else {rep0(j,0,1000)ke[j]=1;}
            rep0(j,0,b)ke[j]^=1;
            for(v99 j=a+b;j<=1000;j+=2*a)
            {
                for(v99 p=j;p<a+j&&p<1000;p++)
                    ke[p]^=1;
            }
            ami[i]=ke;
        }v99 cr_h=0;
