...
void solve()
{
    ...
    rep0(i,0,n)cin>>a[i],b[i]=-1;
    vector<v99>kot;
    rep0(i,0,n)if(a[i]==1)kot.pb(i);
    for(v99 i=0;i<n-2;i++)
    {
        if(kot.empty() and (a[i]==3 or a[i+1]==3 or a[i+2]==3)){
            cout<<-1<<ses;
            return;
        }
        if(a[i]==a[i+1] and a[i+1]==a[i+2] and a[i]==2)
        {
            cout<<-1<<ses;
            return;
        }
    ...
    for(v99 i=0,j=0;i<n && j<kot.size();i++)
    {
        if(a[i]!=1 and i<kot[j])b[i]=kot[j++];
    }
    aout0(b,n);
    vector<pii>vp;
