...
int main()
{
    ...
    //while(1)
    {
        v99 n;cin>>n;string s;cin>>s;bool ok=false;
        map<char,v99>mpp;mpp['a']++;mpp['e']++;mpp['i']++;mpp['o']++;mpp['u']++;mpp['y']++;
        rep0(i,0,n)
        {
            if(ok && mpp[s[i]]){continue;}
            if(mpp[s[i]]){cout<<s[i];ok=true;continue;}
            cout<<s[i];ok=false;
        }cout<<ses;
    }
    return 0;
