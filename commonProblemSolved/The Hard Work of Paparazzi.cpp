...
v99 dfs(v99 ind)
{
    ...
    v99 v=0;
    for(v99 i=ind+1;i<=n;i++)
    {
        v99 val=abs(a[ind].y-a[i].y)+abs(a[ind].z-a[i].z);
        v99 tym=abs(a[ind].x-a[i].x);
        if(tym>=val)v=max(v,dfs(i));
    }
    return dp[ind]=v+1;
}
...
void solve()
{
    /// code is here->
    cin>>r>>n;
    a[0].x=0;a[0].y=1;a[0].z=1;
    rep1(i,1,n)cin>>a[i].x>>a[i].y>>a[i].z;
    memset(dp,-1,sizeof(dp));
    cout<<max(0LL,dfs(0LL)-1)<<ses;
}
