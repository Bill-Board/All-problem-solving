...
void solve()
{
    ...
    vector<pii>vp;v99 i=0;
    while(s[i]=='W')i++;
    while(i<n)
    {
        v99 start=i;
        while(s[i]=='L' && i<n)i++;
        if(i!=n && start!=0)vp.pb(mp(i-start,start));
        while(s[i]=='W' && i<n)i++;
    }
    sort(sob(vp));
    //mout(vp);
    for(auto i:vp)
    {
        v99 st=i.ss,kot=i.ff;
        for(v99 j=st;kot--&&k--;j++)s[j]='W';
        if(k<=0)break;
    }
    if(k>0)
    {
        ...
        if(!koita){cout<<(min(k,n)*2)-1<<ses;return;}
    }
    //cout<<s<<ses;
    v99 cr_h=0;
    for(v99 i=1;i<n;i++)
