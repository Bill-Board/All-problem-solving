...
int main()
{
    ...
    //while(1)
    {
        v99 n,cr_h=0;cin>>n;v99 a[n+2][n+2],k=(n+1)/2;
        rep1(i,1,n){rep1(j,1,n)cin>>a[i][j];}
        rep1(i,1,n)cr_h+=a[i][i];
        rep1(i,1,n)cr_h+=a[k][i];
        rep1(i,1,n)cr_h+=a[i][k];
        for(v99 i=1,j=n;i<=n;i++,j--)cr_h+=a[i][j];
        cout<<cr_h-(3*a[k][k])<<ses;
