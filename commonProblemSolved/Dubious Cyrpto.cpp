...
int main()
{
    ...
    {
        v99 l,r,m;cin>>l>>r>>m;
        v99 lf=l,rt=r,kot=r-l;
        bool ok=false;
        for(v99 i=l;i<=r;i++){
            v99 mid=i;
            v99 rem=m%mid,bah=(m+mid-1)/mid;
            v99 rem2=(bah)*mid - m;
            if(rem2<=kot)
            {
                cout<<mid<<whp<<r-rem2<<whp<<r<<ses;
                break;
            }
            else if(rem<=kot)
            {
                if(bah*mid-rem==m)cout<<mid<<whp<<r-rem<<whp<<r<<ses;
                else cout<<mid<<whp<<r<<whp<<r-rem<<ses;
                ok=true;
                break;
            }
