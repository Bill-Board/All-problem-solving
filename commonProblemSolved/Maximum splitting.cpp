...
{
	memset(prime, true, sizeof(prime));prime[0]=false;
	...
	rep1(i,4,n)if(!prime[i])prm.push_back(i);
...
int main()
{
    ...
    //while(1)
    {
        ...
        v99 n;cin>>n;
        v99 rem=n%4,kot=n/4;
        if((n<=5 && n!=4)|| n==7 ||n==11)cout<<-1<<ses;
        else if(rem==0)cout<<kot<<ses;
        else if(rem==1)cout<<kot-1<<ses;
        else if(rem==2)cout<<kot<<ses;
        else cout<<kot-1<<ses;
    }
    return 0;
}
