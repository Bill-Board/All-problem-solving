...
int main()
{
    ...
    //test 1152921504606846975
    //while(1)
    {
        vector<v99>a;
        v99 q=1;a.push_back(0);
        for(v99 i=1;i<=62;i++)
        {
            a.push_back(q);q*=2;
        }//for(auto i:a)cout<<i<<whp;cout<<ses;
        v99 n,k;cin>>n>>k;v99 nn=n,cr_h=0;
        auto pos=upper_bound(a.begin(),a.end(),n)-a.begin();
        if(k==1)cout<<n<<ses;
        else cout<<a[pos]-1<<ses;
        /*rep0(i,1,a.size())
        {
            if(a[i]<=n && a[i+1]>n)
            {
                if(k==1)cout<<n<<ses;
                else cout<<a[i+1]-1<<ses;
                return 0;
            }
        }*/
