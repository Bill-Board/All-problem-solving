...
{
    /// code is here->
    v99 n,k,s=0;cin>>n>>k;v99 a[n+2];
    map<v99,v99>mpp,kot;
    rep0(i,0,n)
    {
        cin>>a[i];kot[a[i]]++;
        if(i<k)s+=a[i],mpp[a[i]]++;
    }
    v99 cr_h=0,left=0;
    if(mpp.size()==kot.size() or mpp.size()==k)cr_h=s;
    while(k<n)
    {
        s-=a[left];
        s+=a[k];
        if(mpp[a[left]]==1)mpp.erase(a[left]);
        else mpp[a[left]]--;
        mpp[a[k]]++;
        left++;k++;
        if(mpp.size()==kot.size() or mpp.size()==k)cr_h=max(cr_h,s);
        //mout(mpp);cout<<ses<<ses<<ses;
    }
    cout<<cr_h<<ses;
}
