...
int main()
{
    ...
    //while(1)
    {
        v99 n,k,m=10,nn=10;cin>>n>>k;v99 v[10];CLR(v,0);
        rep0(i,0,n){v99 x;cin>>x;v[x]++;m=min(m,x);}
        rep0(i,0,k){v99 x;cin>>x;v[x]++;nn=min(nn,x);}
        rep1(i,1,9)if(v[i]>1)return cout<<i<<ses,0;
        cout<<min(nn,m)<<max(nn,m)<<ses;
    }
    return 0;
}
