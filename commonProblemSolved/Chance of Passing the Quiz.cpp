...
void korchi()
{
    ...
    {
        v99 n=50,r=50;
        for(v99 i=0;i<=n;i++)dyp[i][i]=1;
        for(v99 i=0;i<=n;i++)dyp[i][0]=1;
        for(v99 i=1;i<=n;i++)dyp[0][i]=0;
        for(v99 i=1;i<=n;i++)
        {
            for(v99 j=1;j<i && j<=r;j++)
                dyp[i][j]=dyp[i-1][j]+dyp[i-1][j-1];
        }
    }
}
...
void solve()
{
    ...
    {
        v99 kot=dyp[n][i];
        double p=kot*pow(0.5,(i)*1.0)*pow(0.5,(n-i)*1.0);
        cr_h+=p;
    }
    cout<<fixed<<setprecision(2)<<cr_h<<ses;
}
