...
int berkortesi(int a[],int dp[],vector<int>days,vector<int>cost,int i)
{
    if(i>365)return 0;
    if(dp[i]!=-1)return dp[i];
    if(a[i])
        dp[i]=min({cost[0]+berkortesi(a,dp,days,cost,i+1),cost[1]+berkortesi(a,dp,days,cost,i+7),cost[2]+berkortesi(a,dp,days,cost,i+30)});
    else dp[i]=berkortesi(a,dp,days,cost,i+1);
    return dp[i];
}
int mincostTickets(vector<int>& days, vector<int>& costs) {
        int dp[367],a[367];
        for(int i=0;i<=365;i++)dp[i]=-1,a[i]=0;
        for(auto i:days)a[i]=1;
        return berkortesi(a,dp,days,costs,1);
    }
...
void solve()
{
    ...
    cout<<mincostTickets(days,cost)<<ses;
    //rep1(i,1,30)cout<<dp[i]<<whp;cout<<ses;
}
