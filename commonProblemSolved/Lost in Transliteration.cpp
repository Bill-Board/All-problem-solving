...
///-------------------------------------------------------------------------------------------------------
string berkori(string s)
{
    stack<char>p;
    for(v99 i=0;i<s.size();)
    {
        if(p.empty())p.push(s[i]),i++;
        else if(p.top()=='k' && s[i]=='h')p.pop();
        else p.push(s[i]),i++;
    }
    s.clear();
    while(!p.empty())s+=p.top(),p.pop();
    reverse(sob(s));
    return s;
}
string berkori2(string s)
{
    string p="";
    rep0(i,0,s.size())
    {
...
{
    /// code is here->
    v99 n;cin>>n;map<string ,v99>mpp;
    rep0(i,0,n)
    {
        string s;cin>>s;
        s=berkori(s);
