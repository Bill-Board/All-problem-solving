...
bool kortesi(string a,string b)
{
    int len=a.size()/2;bool ok=true;
    for(int i=0,j=a.size()-1;i<len;i++,j--)
    {
        if(ok && a[i]==b[j])continue;
        ok=false;
...
bool checkPalindromeFormation(string a, string b) {
    ...
    reverse(p.begin(),p.end());reverse(q.begin(),q.end());
        if(kortesi(a,b) || kortesi(b,a))return true;
        if(kortesi(p,q) || kortesi(q,p))return true;
        return false;
    }
void solve()
{
    /// code is here->
    string a,b;cin>>a>>b;
    cout<<checkPalindromeFormation(a,b)<<ses;
}
