...
///-------------------------------------------------------------------------------------------------------
string s;bool ok=false,ans=false;
v99 n,k;
bool berkortesi()
{
    v99 one=0,zero=0;
    ...
    if(dif<one || dif<zero)return false;
    for(v99 i=0,j=k;j<n;j++,i++)
    {
        zero-=(s[i]=='0');
        one-=(s[i]=='1');
        zero+=(s[j]=='0');
...
void solve()
{
    ...
    for(v99 i=0;i<k;i++)
    {
        v99 c0=0,c1=0;char c='?';
        for(v99 j=i;j<n;j+=k)c0+=(s[j]=='0'),c1+=(s[j]=='1');
        if(c0 && c1)
        {
            ok=true;
            break;
        ...
        if(c1)c='1';
        if(c1 || c0)
            for(v99 j=i;j<n;j+=k)s[j]=c;
    }
    if(!ok && berkortesi())cout<<"YES"<<ses;
    else cout<<"NO"<<ses;
}
