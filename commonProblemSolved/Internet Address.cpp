...
void acha(string &d,string &c,string s,v99 i)
{
    while(s[i]!='r' || s[i+1]!='u')
    {
        d+=s[i];
...
{
    /// code is here->
    string s,p="http",c="",d="";cin>>s;
    if(s[0]=='f')p="ftp",d+=s[3],acha(d,c,s,4);
    else d+=s[4],acha(d,c,s,5);
    if(c.size())cout<<p<<"://"<<d<<".ru/"<<c<<ses;
    else cout<<p<<"://"<<d<<".ru"<<ses;
}
