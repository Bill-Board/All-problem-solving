...
///-------------------------------------------------------------------------------------------------------
long double loop=1e-9;
v99 n;
vll m,v;

bool alabu(long double minite)
{
    long double a,b;
    rep0(i,0,n)
    {
        long double r=m[i]+(v[i]*1.0*minite),l=max(0.0L,m[i]-(v[i]*1.0*minite));
        if(i==0){a=l;b=r;continue;}
