...
{
    /// code is here->
    v99 n,m=0;cin>>n;v99 a[n+1];
    deque<v99>d;
    rep0(i,0,n)cin>>a[i],m=max(m,a[i]);
    if(n<=2)
    {
        cout<<m<<ses;
        return;
    ...
    sort(a,a+n,greater<>());
    rep0(i,0,n)d.push_back(a[i]);
    v99 ans=0;
    while(!d.empty())
    {
        v99 ek=d.front(),dui=0;d.pop_front();
        if(!d.empty())dui=d.front(),d.pop_front();
        if(dui==0)ans+=ek;
        else if(ek!=dui)
        {
            v99 k=min(ek,dui);ek-=k;dui-=k;
            ans+=k;
            if(ek)d.push_front(ek);
            else if(dui)d.push_front(dui);
        }
        else ans+=ek;
    }
    cout<<ans<<ses;
