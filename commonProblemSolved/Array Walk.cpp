...
{
    /// code is here->
    v99 n,k,z,sum=0,cr_h=0;cin>>n>>k>>z;v99 a[n+2],b[n+1],f[z+1];f[0]=0;b[0]=0;
    rep0(i,1,n){cin>>a[i];b[i]=b[i-1]+a[i];if(i<=k)sum+=a[i];}v99 j=k;
    rep1(i,1,z)f[i]=f[i-1]+a[j]+a[j-1],j-=2,cout<<f[i]<<ses;cr_h=a[1];
    rep1(i,2,k+1)
    {
        cr_h+=a[i];
    ...
    cr_h=a[0];cout<<sum<<ses;
    rep0(i,1,k-1)
    {
        v99 kot=a[i]+a[i-1];
        for(v99 j=1;j<=min((v99)k-i,z);j++)
        {
            cout<<whp<<kot<<whp<<j<<whp<<f[j]<<ses;
            sum=max(sum,sum+(kot*j)-f[j]);
            cout<<sum<<ses;
        }
    }
    cout<<sum<<ses;
