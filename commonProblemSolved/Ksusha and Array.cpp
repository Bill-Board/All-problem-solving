...
int main()
{
    ...
    //while(1)
    {
        v99 n,cr_h;cin>>n;map<v99,v99>mpp;
        rep0(i,0,n){v99 x;cin>>x;mpp[x]++;}bool ok=false;
        for(auto i:mpp)
        {
            if(!ok){cr_h=i.first;ok=true;}
            else cr_h=gcd(cr_h,i.first);
        }
        if(mpp[cr_h])cout<<cr_h<<ses;
        else cout<<-1<<ses;
    }
    return 0;
}
