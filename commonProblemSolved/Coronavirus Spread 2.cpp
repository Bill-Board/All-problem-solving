...
void init(v99 n)
{
    rrr.clear();
    sze.clear();
    rep1(i,1,n){rrr[i]=i;sze[i]=1;tom[i].clear();}
}
void solve()
{
    /// code is here->
    v99 n;cin>>n;v99 a[n+2],v[n+2];
    rep1(i,1,n)cin>>a[i],v[i]=i;
    init(n);
    v99 kw=10,kb=0;
    rep1(i,1,100)
    {
        rep1(j,1,n)v[j]+=a[j],tom[i].pb(v[j]);
    }
    rep1(i,1,100)
    {
        for(auto j:tom[i])cout<<j<<whp;cout<<ses;
    }
    rep1(i,1,100)
    {
        for(v99 j=0;j<n-1;j++)
        {
            for(v99 k=j+1;k<n;k++)
                if(tom[i][j]==tom[i][k])dsu(j+1,k+1);
        }
    }
    map<v99,v99>mpp;
    rep1(i,1,n)mpp[root(i)]++;
    for(auto i:mpp)kw=min(kw,i.ss),kb=max(kb,i.ss);
    cout<<kw<<whp<<kb<<ses;
}
