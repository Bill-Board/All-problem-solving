...
using namespace std;
v99 fx[8]= {1,-1,1,-1,2,-2,0,0};
v99 fy[8]= {1,1,-1,-1,0,0,2,-2};
...
{
    /// code is here->
    v99 n,m,x,y,cr_h;cin>>n>>m>>x>>y;
    if(m==1 && n==1){cout<<x<<ses;return;}
    v99 fst=min(x,y),last=min(x,y-fst);
    v99 big=max(fst,last),chota=min(fst,last);
    fst=((n*m)+1)/2;
    last=(n*m)/2;
    cout<<fst*big + last*chota<<ses;
    return;
}
