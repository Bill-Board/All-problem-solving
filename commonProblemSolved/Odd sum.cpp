...
int main()
{
    ...
    //while(1)
    {
        v99 n,posmin=LLONG_MAX,negmin=-LLONG_MAX;cin>>n;
        v99 sum=0,a[n+2];
        rep0(i,0,n)
        {
            cin>>a[i];if(a[i]>0)sum+=a[i];
            if(a[i]>0 && posmin>a[i] && a[i]&1)posmin=a[i];
            if(a[i]<0 && negmin<a[i] && a[i]&1)negmin=a[i];
        }
        if(sum&1)cout<<sum<<ses;
        else if(posmin!=LLONG_MAX && negmin!=(-LLONG_MAX))
        {
            sum=max(sum-posmin,sum+negmin);
            cout<<sum<<ses;
        }
        else if(posmin!=LLONG_MAX)cout<<sum-posmin<<ses;
        else cout<<sum+negmin<<ses;
    }
    return 0;
}
