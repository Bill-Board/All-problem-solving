...
int main()
{
    ...
    //while(1)
    {
        v99 n,m;cin>>n>>m;v99 cr_h[n+1][m+1],a[n+1][m+1],r[n+1],c[m+1];CLR(cr_h,0);
        rep0(i,0,n)
        {
            v99 sum=0;
            rep0(j,0,m)
        ...
        rep0(i,0,n)
        {
            rep0(j,0,m)
            {
                ...
                {
                    qq++;
                    if(r[i]!=m && c[j]!=n)ok=true;
                    else if(r[i]==m && c[j]==n)cr_h[i][j]=1,q=0;
                }
            }
        }
        if(ok || (q&&qq))cout<<"NO"<<ses;
        else
        {
            cout<<"YES"<<ses;
            rep0(i,0,n)
            {
                rep0(j,0,m)cout<<cr_h[i][j]<<whp;
                cout<<ses;
            }
        }
