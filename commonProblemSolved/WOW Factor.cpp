...
{
    /// code is here->
    string s;cin>>s;s+='o';v99 n=s.size();
    v99 i=0,sum=0;bool ok=false;vector<v99>k;
    while(s[i]=='o')i++;
    while(i<n)
    {
        if(ok && s[i]=='o'){if(sum)k.pb(sum);sum=0;k.pb(0);}
        else if(s[i]=='v' && s[i+1]=='v')sum++,ok=true;
        i++;
    }
    if(k.empty()){cout<<0<<ses;return;}
    v99 pre[n+1],suf[n+1];sum=0;
    rep0(i,0,k.size())sum+=k[i],pre[i]=sum;sum=0;
    for(v99 i=k.size()-1;i>=0;i--)sum+=k[i],suf[i]=sum;
    v99 cr_h=0;
    rep0(i,0,k.size())if(k[i]==0)cr_h+=(suf[i]*pre[i]);
    cout<<cr_h<<ses;
}
