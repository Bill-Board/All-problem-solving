...
void setout(set<v99>a)
{
    for(v99 i:a)cout<<i<<whp;cout<<ses;
}
void mout(auto a)
{
...
void solve()
{
    ...
    set<v99>x,y;
    for(v99 i=a;i<=min(b,a+1);i++)x.insert(i);
    for(v99 i=b;i>=max(a,b-1);i--)x.insert(i);
    for(v99 i=c;i<=min(d,c+1);i++)y.insert(i);
    for(v99 i=d;i>=max(c,d-1);i--)y.insert(i);
    v99 cr_h=-LLONG_MAX;
    //for(v99 i:x)cout<<i<<whp;cout<<ses;
    //for(v99 i:y)cout<<i<<whp;cout<<ses;
    for(v99 i:x)
    {
        for(v99 j:y)cr_h=max(cr_h,i*j);
    }
    cout<<cr_h<<ses;
}
