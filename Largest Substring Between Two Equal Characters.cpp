...
///-------------------------------------------------------------------------------------------------------
int kot(string s,char a,char b)
{
    int len=0,m=0;
    for(int i=0;i<s.size();i++)
    {
        if(s[i]==a or s[i]==b)len++;
        else m=max(m,len),len=0;
    }m=max(m,len);
    return m;
}
...
int maxLengthBetweenEqualCharacters(string s) {
        ...
        rep0(i,0,s.size())mpp[s[i]].pb(i);
        for(auto i:mpp)
        {
            vector<int>v=i.ss;
            if(v.size()==1)continue;
            len=max(len,v[v.size()-1]-v[0]-1);
        }
        return len;
    }
...
void solve()
{
    /// code is here->
    string s;cin>>s;
    cout<<maxLengthBetweenEqualCharacters(s)<<ses;
