...
int main()
{
    ...
    //while(1)
    {
        v99 n;string s,cr_h="",ans="";cin>>n>>s;
        v99 i=0,j=n-1,one=0,z=0;
        for(v99 i=0;i<n;i++)
        {
            if(s[i]=='1')one++;
            else z++;
        }
        if(one==n || z==n){cout<<s<<ses;continue;}
        while(s[i]!='1' && i<n){cr_h+=s[i];i++;}
        while(s[j]!='0' && j>=0){ans+=s[j];j--;}
        if(i==j+1)cout<<cr_h<<ans<<ses;
        else cout<<cr_h<<"0"<<ans<<ses;
    }
    return 0;
}
