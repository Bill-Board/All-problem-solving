...
{
    /// code is here->
    v99 n,cr_h=0,ma=LLONG_MAX,mb=LLONG_MAX;cin>>n;v99 a[n+1],b[n+1];
    rep0(i,0,n)cin>>a[i],ma=min(ma,a[i]);
    rep0(i,0,n)cin>>b[i],mb=min(mb,b[i]);
    for(v99 i=0;i<n;)
    {
        if(a[i]>ma && b[i]>mb)
        {
        ...
        else if(a[i]>ma)cr_h+=(a[i]-ma),i++;
        else if(b[i]>mb)cr_h+=(b[i]-mb),i++;
        else i++;
        //cout<<i<<ses;
    }
    cout<<cr_h<<ses;
