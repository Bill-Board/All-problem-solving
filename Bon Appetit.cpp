...
vector<pii>tom[mxi];
map<v99,v99>mpp;
v99 berkortesi_to(vector<pii>a)
{
    if(a.empty())return 0;
    sort(sob(a));
    v99 cr_h=1,left=a[0].ff;
    for(v99 i=1;i<a.size();i++)
    {
        if(a[i].ss>left)cr_h++,left=a[i].ff;
    }
    return cr_h;
}
void jharu()
...
{
    /// code is here->
    v99 n,k,pos=0;cin>>n>>k;
    rep0(i,0,n)
    {
        v99 l,r,val;cin>>l>>r>>val;
        if(mpp.find(val)==mpp.end())mpp[val]=pos++;
        tom[mpp[val]].pb(mp(r-1,l));
    }
    v99 cr_h=0;
    rep0(i,0,pos)cr_h+=berkortesi_to(tom[i]);
    cout<<cr_h<<ses;
}
