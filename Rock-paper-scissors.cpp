...
v99 solve()
{
    ...
    if(f[0]=='r' && m[0]=='s' && s[0]=='s')return cout<<"F"<<ses,0;
    if(f[0]=='s' && m[0]=='p' && s[0]=='p')return cout<<"F"<<ses,0;
    if(f[0]=='p' && m[0]=='r' && s[0]=='r')return cout<<"F"<<ses,0;

    if(f[0]=='r' && m[0]=='p' && s[0]=='r')return cout<<"M"<<ses,0;
    if(f[0]=='p' && m[0]=='s' && s[0]=='p')return cout<<"M"<<ses,0;
    if(f[0]=='s' && m[0]=='r' && s[0]=='s')return cout<<"M"<<ses,0;

    if(f[0]=='p' && m[0]=='p' && s[0]=='s')return cout<<"S"<<ses,0;
    if(f[0]=='r' && m[0]=='r' && s[0]=='p')return cout<<"S"<<ses,0;
    if(f[0]=='s' && m[0]=='s' && s[0]=='r')return cout<<"S"<<ses,0;
    cout<<"?"<<ses;
