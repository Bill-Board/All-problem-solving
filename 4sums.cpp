...
{
    vector<vector<int> >cr_h;
    int n=nums.size();
    map<int,vector<pair<int ,int > > >mpp;
    for(int i=0;i<n-1;i++)
    {
        for(int j=i+1;j<n;j++)mpp[nums[i]+nums[j]].push_back(mp(i,j));
    }
    set<vector<int> >ans;
    for(int i=0;i<n-1;i++)
    {
        for(int j=i+1;j<n;j++)
        {
            if(mpp.find(target-(nums[i]+nums[j]))==mpp.end())continue;
            for(auto k:mpp[target-(nums[i]+nums[j])])
            {
                int a=k.first,b=k.second;
                if(a!=i and a!=j and b!=i and b!=j)
                {
                    vector<int>pp({nums[i],nums[j],nums[a],nums[b]});
    ...
    for(auto i:ans)cr_h.push_back(i);
    return cr_h;
}
