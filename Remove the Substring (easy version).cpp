...
v99 backwrd[mxi],forwrd[mxi];
v99 solve()
{
    /// code is here->
    string s,t;cin>>s>>t;
    CLR(backwrd,-1);CLR(forwrd,-1);
    for(v99 i=0,j=0;j<t.size();i++,j++)
    {
        while(s[i]!=t[j])i++;
        forwrd[j]=i;
    }
    for(v99 i=s.size()-1,j=t.size()-1;j>=0;i--,j--)
    {
        while(s[i]!=t[j])i--;
        backwrd[j]=i;
    }
    //aout0(forwrd,t.size());
    //aout0(backwrd,t.size());
    v99 cr_h=s.size()-forwrd[t.size()-1]-1;
    cr_h=max(cr_h,backwrd[0]);
    rep0(i,0,t.size()-1)cr_h=max(cr_h,backwrd[i+1]-forwrd[i]-1);
    cout<<cr_h<<ses;
}
