...
#define i_love_u_huu IOS long long t;cin >> t;while(t--)
#define ses   "\n"
#define whp   " "
#define mxi 200003
#define mp make_pair
...
v99 solve()
{
    ...
    rep0(i,0,vp.size()-1)
    {
        double dif=vp[i+1].ff-(vp[i+1].ss/2.0)-vp[i].ff-(vp[i].ss/2.0);
        //cout<<"dif "<<dif<<ses;
        if(dif>(double)t)cr_h+=2;//,cout<<dif-t+1<<ses;
        else if(abs(dif-(double)t)<0.000001)cr_h++;
    }cout<<cr_h+2<<ses;
}
