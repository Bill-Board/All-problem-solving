...
int main()
{
    ...
    //while(1)
    {
        v99 n;cin>>n;v99 a[2*n+4],visi[n+2];CLR(visi,0);
        rep0(i,0,2*n)cin>>a[i];vector<v99>cr_h;
        for(v99 i=2*n-1;i>=0;i--)
        {
            if(visi[a[i]]==0)
            {
                visi[a[i]]=1;
        ...
        }reverse(cr_h.begin(),cr_h.end());
        for(auto i:cr_h)cout<<i<<whp;cout<<ses;
    }
    return 0;
}
