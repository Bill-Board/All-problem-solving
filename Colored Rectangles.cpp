...
///-------------------------------------------------------------------------------------------------------
v99 dp[201][201][201],r,g,b;
vector<v99>R,G,B;

v99 kortesi(v99 rr,v99 bb,v99 gg)
{
    if(dp[rr][bb][gg]!=-1)return dp[rr][bb][gg];
    v99 mx=0;
    if(rr<r and bb<b)mx=max(mx,(R[rr]*B[bb])+kortesi(rr+1,bb+1,gg));
    if(gg<g and bb<b)mx=max(mx,(G[gg]*B[bb])+kortesi(rr,bb+1,gg+1));
    if(rr<r and gg<g)mx=max(mx,(R[rr]*G[gg])+kortesi(rr+1,bb,gg+1));
    return dp[rr][bb][gg]=mx;
}
...
void solve()
{
    ...
    rep0(i,0,r){v99 x;cin>>x; R.pb(x);}
    rep0(i,0,g){v99 x;cin>>x; G.pb(x);}
    rep0(i,0,b){v99 x;cin>>x; B.pb(x);}
    sort(sob(R),greater<v99>());
    sort(sob(G),greater<v99>());
    sort(sob(B),greater<v99>());
    cout<<kortesi(0LL,0LL,0LL)<<ses;
}
