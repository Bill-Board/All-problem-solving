...
{
    /// code is here->
    v99 n,k,m=-LLONG_MAX,l=-LLONG_MAX;cin>>n>>k;v99 p[n+1];
    vector<v99>a,b;
    rep0(i,0,n)cin>>p[i],m=max(p[i],m);
    rep0(i,0,n)a.pb(m-p[i]),l=max(a[i],l);
    rep0(i,0,n)b.pb(l-a[i]);
    if(k&1)for(auto i:a)cout<<i<<whp;
    else for(auto i:b)cout<<i<<whp;
    cout<<ses;
