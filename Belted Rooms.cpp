...
vector<v99>tom[300001];
void solve()
{
    /// code is here->
    v99 n,cr_h=0;cin>>n;v99 visi[n+1]={};
    string s;cin>>s;v99 k=0,kk=0;
    for(v99 i=0;i<n;i++){
            if(s[i]=='>')k++;
            if(s[i]=='<')kk++;
            v99 samne=(i+1)%n;
            if(s[i]=='-')visi[i]=visi[samne]=1;
    }
    if(!k or !kk){cout<<n<<ses;return;}
    for(v99 i=0;i<n;i++)if(visi[i])cr_h++;
    cout<<cr_h<<ses;
