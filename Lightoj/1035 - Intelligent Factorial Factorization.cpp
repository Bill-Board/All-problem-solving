...
int main()
{
    ...
    rep1(i,1,100)
    {
        ...
        for(auto j:prm)
        {
            ...
            {
                v99 cnt=0;
                while(nn%j==0){cnt++;nn/=j;}
                tom[i].push_back({j,cnt});
            }
            if(nn==1)break;
        }
    }
    ...
    //while(1)
    {
        ...
        rep1(i,1,n)
        {
            for(auto j:tom[i])mpp[j.first]+=j.second;
        }v99 j=0;
        printf("Case %lld: %lld = ",q++,n);
        for(auto i:mpp){
                printf("%lld (%lld)",i.first,i.second);j++;
                //cout<<i.first<<" ("<<i.second<<") ";
                if(j!=mpp.size())printf(" * ");}
        printf("\n");
