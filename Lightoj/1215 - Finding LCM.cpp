...
///-------------------------------------------------------------------------------------------------------
void facto(v99 a,map<v99,v99>&ek)
{
    for(auto i:prm)
        {
            if(a==1)break;
...
int main()
{
    ...
    SieveOfEratosthenes(1000000);v99 q=1;
    test
    //while(1)
    {
        v99 a,b,l;cin>>a;cin>>b;cin>>l;
        map<v99,v99>ek,tin;a=lcm(a,b);///cout<<a<<ses;
        facto(a,ek);facto(l,tin);
        v99 cr_h=1;bool ok=false;
        for(auto i:tin)
        {
            v99 kot=i.first,koita=i.second;
            if(ek[kot]==koita)continue;
            cr_h*=pw(kot,koita);
        }
        if(lcm(a,cr_h)!=l)printf("Case %lld: impossible\n",q++);
