...
void init()
{
    ...
    rep1(i,1,12)
    {
        q*=5;
        mpp[q/5]=i;
        rep0(j,1,i)mpp[q/5+j]=i;
    }
    //for(auto i:mpp)cout<<i.first<<whp<<i.second<<ses;cout<<ses;
}

void solve(v99 n,v99 q)
{

    if(mpp[n]){printf("Case %lld: impossible\n",q);return;}
    v99 p;
    for(auto i:mpp)
    {
        if(i.first<n){p=i.second;break;}
    }
    v99 k=(p *(p +1))/2;if(n<5)k=0;
    printf("Case %lld: %lld\n",q,(n-k)*5);
