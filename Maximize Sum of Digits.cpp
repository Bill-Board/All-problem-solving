...
///-------------------------------------------------------------------------------------------------------
vector<pii>v(mxi);
void acha(v99 n,v99 node)
{     /// ff hoise mimimum value, ss hoise kot.
    //cout<<n<<whp<<node<<ses;
    if(n<10){v[node].ss=v[node].ff=n;return;}
    acha(n/10,2*node);acha(n/10 -1,2*node+1);
    v99 x=v[2*node].ff+n%10, y=v[2*node +1].ff+9;
    v[node].ff=max(x,y);
    v[node].ss=x<y ? v[2*node+1].ss*10 + 9: v[2*node].ss*10 +(n%10);
}
