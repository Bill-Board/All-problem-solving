...
///-------------------------------------------------------------------------------------------------------
v99 n,k,q=0,p=0,ans[mxi];
void combi(v99 koita_hoise,v99 last_konta)
{
    if(koita_hoise==k+1)
    {
        q++;
        rep1(i,1,k)cout<<ans[i]<<whp ses[i==k];
        return;
    }p++;

    for(v99 i=last_konta+1;i<=(n-k+koita_hoise);i++)
    {
        ans[koita_hoise]=i;
        combi(koita_hoise+1,i);
    }
}
