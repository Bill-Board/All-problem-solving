...
///-------------------------------------------------------------------------------------------------------
vector<v99>o,z;v99 a,b;
void hoise(string s)
{
    v99 n=s.size(),q=0,l=n-1;
    while(s[q]=='0' and q<n)q++;
    while(s[l]=='0' and l>=0)l--;
    while(q<=l)
    {
        v99 ko=0,kz=0;
        while(s[q]=='1' and q<=l)q++,ko++;
        while(s[q]=='0' and q<=l)q++,kz++;
        if(ko)o.pb(ko);
        if(kz)z.pb(kz);
    }
}
...
void solve()
{
    ...
    o.clear();z.clear();
    v99 n=s.size();
    hoise(s);v99 visi[z.size()+4]={};
    v99 cr_h=0;
    if(o.empty()){cout<<0<<ses;return;}
