...
v99 alabu(v99 val)
{
    ...
    {
        v99 sum=1;
        rep0(j,0,prm.size())if(i&(1<<j))sum*=prm[j];
        if(sum==1)continue;
        //print("sum",sum);
        if(__builtin_popcount(i) &1)  /// odd gula jog krbo
        {
            v99 kot=(val/sum);
            odd+=kot;
        }
...
{
    /// code is here->
    v99 a,b,m;read(a,b,m);
    if(a>b)swap(a,b);
    v99 dif=(b-a),dif2=(b-a),c=a+m,d=b+m,pro=1;
    if(a==1 and b==1){cout<<"Case "<<tc++<<": "<<1<<ses; return;}
    if(a==b){cout<<"Case "<<tc++<<": "<<0<<ses; return;}
    if(dif==1){cout<<"Case "<<tc++<<": "<<m+1<<ses; return;}
    for(auto i:pr)
    {
        if(dif2%i==0)
        {
            while(dif2%i==0)dif2/=i;
            prm.pb(i);pro*=i;
        }
        if(dif2==1)break;
    }
    if(dif2!=1)prm.pb(dif2),pro*=dif2;
    v99 pic_a=(a+pro-1)/pro;pic_a*=pro;
    v99 cr_h=0;
    //print(prm);
    if(pic_a<=c)
    {
        cr_h+=( alabu(c-pic_a)+alabu(pic_a-a) +1);
        //print(a,pic_a,c,"YES",alabu(c-pic_a),alabu(pic_a-a));
    }
    else
    {
        cr_h+=(alabu(pic_a-a)-alabu(pic_a-c-1));
        //print(a,pic_a,c,"YES",alabu(pic_a-a),alabu(pic_a-c-1));
    }
    cout<<"Case "<<tc++<<": "<<max(0LL,m+1-cr_h)<<ses;
    prm.clear();
}
