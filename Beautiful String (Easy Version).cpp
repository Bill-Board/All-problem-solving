...
v99 dp[1000001][27],dp2[1000002][27];
...
void solve()
{
    ...
    rep0(j,13,26)
    {
        rep0(i,1,n)dp2[i][j]+=dp2[i-1][j];
    }
    for(v99 j=25;j>12;j--)
    {
        for(v99 i=n-1;i>=0;i--)dp[i][j]+=dp[i+1][j];
    }
    for(v99 j=0;j<13;j++)
    {
        for(v99 i=n-1,ii=1;i>0;ii++,i--)cr_h+=(dp2[i][j]*dp2[i-1][f[j]]),cr_h+=(dp[ii][j]*dp[ii+1][f[j]]);
    }
    cout<<cr_h<<ses;
