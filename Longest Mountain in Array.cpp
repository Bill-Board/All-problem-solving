...
int longestMountain(vector<int>& arr) {
        int n=arr.size(),cr_h=0;
        int pic[n+2],sam[n+2];
        for(int i=0;i<=n;i++)pic[i]=sam[i]=0;
        for(int i=1;i<n;i++)if(arr[i]>arr[i-1])pic[i]+=pic[i-1]+1;
        for(int i=n-2;~i;i--)if(arr[i]>arr[i+1])sam[i]+=sam[i+1]+1;
        for(int i=0;i<n;i++)if(pic[i] and sam[i])cr_h=max(cr_h,pic[i]+sam[i]+1);
        if(cr_h>=3)return cr_h;
        else return 0;
    }
