...
///-------------------------------------------------------------------------------------------------------
v99 n,a[201],dp[402][204];
v99 koita(v99 tym,v99 pos)
{
    if(n<=pos)return 0;
    if(tym>400)return mxi;
    if(dp[tym][pos]!=-1)return dp[tym][pos];
    return dp[tym][pos]=min(koita(tym+1,pos),abs(tym-a[pos])+koita(tym+1,pos+1));
}
