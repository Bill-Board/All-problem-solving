...
int main()
{
    ...
    //while(1)
    {
        v99 n,k,cr_h=0;cin>>n>>k;
        v99 a[n+3],b[k+3];vector<v99>neg,pos;
        rep0(i,0,n){cin>>a[i];if(a[i]<0)neg.push_back(a[i]);
        else pos.push_back(a[i]);}
        rep0(i,0,k)cin>>b[i];
        sort(a,a+n);sort(b,b+k);sort(neg.begin(),neg.end());reverse(a,a+n);
        sort(pos.begin(),pos.end());v99 koita=0;
        while(b[koita]==1 && koita<k)koita++;
        for(v99 i=0;i<k;i++)
        {
            cr_h+=a[i];
            if(koita)
        ...
        reverse(a,a+n);reverse(b,b+k);v99 j=0;
        for(v99 i=0;i<k;i++)
        {
            v99 ha=b[i]-1;
            if(ha)cr_h+=a[j];
