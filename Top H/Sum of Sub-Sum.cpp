...
v99 fact[N+3],inv[N+3];
void precal()
{
    fact[0]=1;
    rep1(i,1,N) fact[i]=modMul(fact[i-1],i);
    ///rep1(i,0,N) inv[i]=bigmod(fact[i],mod-2,mod);   ///emne mod ber kora jay
    inv[N]=bigmod(fact[N],MOD-2,MOD);
    for(v99 i=N-1;~i;--i)inv[i]=modMul(inv[i+1],(i+1)); /// ebaveo kora jay
}
v99 ncr(v99 n,v99 r)
{
    if(n<r)return 0LL;
    v99 kot=modMul(fact[n],inv[n-r]);
    return modMul(kot,inv[r]);
}
...
void solve()
{
    ...
    vll a(n);read(a);
    for(v99 i=m;i<=n;i+=m)
        cr_h=modAdd(cr_h,ncr(n-1,i-1));
    //print(cr_h);
    v99 ans=0;
    for(auto i:a)ans=modAdd(ans,modMul(i,cr_h));
    print(ans);
