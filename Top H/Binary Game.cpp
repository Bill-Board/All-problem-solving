...
///-------------------------------------------------------------------------------------------------------
bool is_bijor(v99 one,v99 z,v99 s)
{
    if(one==0 or s>=one)return false;
    if(z==0)return (one-s)%2==1;
    if((one+z)==s)return false;
    return true;
}
bool is_jor(v99 one,v99 z,v99 s)
{
    if(one<=s)return true;
    if(z==0)return (one-s)%2==0;
    return true;
}
...
void solve()
{
    ...
    v99 n,z=0,k;read(n);
    vll a(n);
    for(auto &i:a)read(i),z+=(i==0);
    read(k);
    v99 o=n-z;
    v99 L=(k+1)/2,R=(k/2);
    if(o==0 or n==k)return print("A6pap");
    if(o==n)
    {
        if((o-k)%2==0)print("A6pap");
        else print("L_Man");
        return;
    }
    bool kar=(k%2==1);
    ...
    while(k--)
    {
        ...
        {
            v99 lzero=L-lone;
            v99 baki_one=o-lone,baki_z=z-lzero;
            if(is_bijor(baki_one,baki_z,R))return print("L_Man");
            lone=min(o,lone+1);
        }
        else/// A6pap
        {
            v99 lzero=L-lone;
            v99 baki_one=o-lone,baki_z=z-lzero;
            if(is_jor(baki_one,baki_z,R))return print("A6pap");
            lone=min(o,lone+1);
        }
    }
    if(kar)print("A6pap");
