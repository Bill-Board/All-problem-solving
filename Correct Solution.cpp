...
{
    /// code is here->
    string s,s1,m,m1;cin>>s>>m;s1=s;m1=m;
    if(s=="0" && m=="0"){cout<<"OK"<<ses;return;}
    sort(s1.begin(),s1.end());sort(m1.begin(),m1.end());
    if(s1!=m1){cout<<"WRONG_ANSWER"<<ses;return;}
    if(s1[0]!='0' && s1!=m){cout<<"WRONG_ANSWER"<<ses;return;}

    string p="",q="",w="";v99 i;
    for(i=0;s1[i]=='0' && i<s.size();i++)p+=s1[i];
    v99 j=i;
    while(s1[j]==s1[i] && i<s.size())q+=s1[i],i++;
    while(i<s.size())w+=s1[i],i++;
    char c=q[0];q.erase(q.begin());
    string ans=c+p+q+w;//cout<<ans<<ses;
    if(ans==m)cout<<"OK"<<ses;
    else cout<<"WRONG_ANSWER"<<ses;
}
