...
{
    /// code is here->
    string s;cin>>s;v99 dot=0,n=s.size();
    if(s[0]=='.'||s[n-1]=='.')return cout<<"NO"<<ses,0;
    s+='.';n=s.size();
    vector<string>sp,cr;string p="";
    for(v99 i=0;i<n;i++)
    {
        if(s[i]=='.')dot++;
        if(s[i]=='.'&& p.empty())return cout<<"NO"<<ses,0;
        if(s[i]=='.')sp.pb(p),p="";
        else p+=s[i];
    }
    if(dot==1)return cout<<"NO"<<ses,0;
    if(sp.empty())return cout<<"NO"<<ses,0;
    if(sp[sp.size()-1].size()>3)return cout<<"NO"<<ses,0;
    for(v99 i=0;i<sp.size()-1;i++)
    {
        string q=sp[i];
        string r="",b="";
        if(sp[i].empty())return cout<<"NO"<<ses,0;
        if(i==0 && sp[i].size()>8)return cout<<"NO"<<ses,0;
        if(i==sp.size()-2)
        {
            cr.pb(q+"."+sp[i+1]);
            continue;
        ...
        if(sp[i+1].size()>3)
        {
            for(v99 j=0;j<3;j++)r+=sp[i+1][j];
            for(v99 j=3;j<sp[i+1].size();j++)b+=sp[i+1][j];
        }
        else if(sp[i+1].empty())return cout<<"NO"<<ses,0;
        else
        {
            r+=sp[i+1][0];
            for(v99 j=1;j<sp[i+1].size();j++)b+=sp[i+1][j];
        }
        if(b.empty())return cout<<"NO"<<ses,0;
        sp[i+1]=b;
        cr.pb(q+"."+r);
    }
    for(auto i:cr)
    {
        v99 age=0,pore=0;bool ok=false;
        for(auto j:i)
        {
            if(j=='.')ok=true;
            else if(ok)pore++;
        ...
        if(age==0 || pore==0)return cout<<"NO"<<ses,0;
        if(age>8 || pore>3)return cout<<"NO"<<ses,0;
    }
    cout<<"YES"<<ses;
    for(auto i:cr)cout<<i<<ses;
}
