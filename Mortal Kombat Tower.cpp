...
///-------------------------------------------------------------------------------------------------------
v99 n,a[mxi],dp[2][mxi];

v99 dfs(v99 ind,v99 val)
{
    if(ind>=n)return 0LL;
    if(dp[val][ind]!=-1)return dp[val][ind];
    if(val)
    {
        v99 dif=dfs(ind+1,val^1);
        v99 kot=(a[ind]==1);
        v99 kot2=(a[ind]==1)+(a[ind+1]==1);
        v99 dif2=dfs(ind+2,val^1);
        dp[val][ind]=min(dif+kot,dif2+kot2);
    }
    else
    {
        v99 dif=dfs(ind+1,val^1);
        v99 dif2=dfs(ind+2,val^1);
        dp[val][ind]=min(dif2,dif);
    }
    return dp[val][ind];
}
...
void solve()
{
    /// code is here->
    cin>>n;
    rep0(i,0,n)cin>>a[i],dp[0][i]=dp[1][i]=-1;
    cout<<dfs(0LL,1LL)<<ses;
    /*rep0(i,0,2)
    {
        rep0(j,0,n)cout<<dp[i][j]<<whp;
        cout<<ses;
    }*/
