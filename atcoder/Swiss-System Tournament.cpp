...
v99 win(char a,char b)
{
    if(a=='G' and b=='C')return 1LL;
    if(a=='C' and b=='P')return 1LL;
    if(a=='P' and b=='G')return 1LL;
    if(a==b)return 0LL;
    return -1LL;
}
...
void solve()
{
    ...
    v99 n,m;read(n,m);
    char s[2*n+2][m+2];
    rep1(i,1,2*n)rep1(j,1,m)read(s[i][j]);
    vector<pii>vp(2*n+1);
    rep1(i,1,2*n) vp[i].ss=i,vp[i].ff=0LL;
    for(v99 j=1;j<=m;j++)
    {
        for(v99 i=1;i<=2*n;i+=2)
        {
            v99 id1=vp[i].ss,id2=vp[i+1].ss;
            v99 kot=win(s[id1][j],s[id2][j]);
            if(kot==1)vp[i].ff++;
            else if(kot==-1)vp[i+1].ff++;
        }
        sort(vp.begin()+1,vp.end(),sort2val);
        //for(auto i:vp)print(i.ff,i.ss);print("");
    }
    rep1(i,1,2*n)print(vp[i].ss);
}
