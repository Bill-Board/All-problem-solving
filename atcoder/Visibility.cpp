...
{
    /// code is here->
    v99 h,w,x,y;read(h,w,x,y);
    char s[h+2][w+2];
    rep1(i,1,h)rep1(j,1,w)read(s[i][j]);
    v99 cr_h=0;
    v99 i=x,j=y;
    while(s[i][j]==s[x][y] and i<=h)cr_h++,i++;
    i=x;
    while(s[i][j]==s[x][y] and i>0)cr_h++,i--;
    i=x;
    while(s[i][j]==s[x][y] and j<=w)cr_h++,j++;
    i=x,j=y;
    while(s[i][j]==s[x][y] and j>0)cr_h++,j--;
    print(cr_h-3);
}
