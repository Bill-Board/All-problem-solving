...
bool ok(v99 val)
{
    ...
    v99 sumx=0,sumy=0;//print("ok");
    for(auto i:a)
    {
        if(pq.size()==val)
        {
            sumx-=pq.top().ss;sumy-=pq.top().ff;
            pq.pop();
        }
        sumx+=i.ff;sumy+=i.ss;
        pq.push({i.ss,i.ff});
        if(sumx>=x and sumy>=y)return true;
    }
    return false;
}
...
{
    /// code is here->
    v99 n,ek=0,dui=0;read(n,x,y);
    a.resize(n);
    for(v99 i=0;i<n;i++)read(a[i].ff,a[i].ss),ek+=a[i].ff,dui+=a[i].ss;
    if(ek<x or dui<y)return print(-1);
    sort(sob(a),sort2val);
    v99 l=1,r=n;
    while(l<r)
    {
        v99 mid=(l+r)/2;
        if(ok(mid))r=mid;//,print(mid);
        else l=mid+1;
    }
    print(r);
}
