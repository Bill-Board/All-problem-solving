...
string s;
map<string,v99>mpp;
bool ok(v99 i,v99 j,v99 k,v99 l)
{
    rep0(p,0,s.size())
    {
        if(s[p]=='o' and ( i!=p and j!=p and k!=p and l!=p ))return false;
        if(s[p]=='x' and( i==p or j==p or k==p or l==p  ) )return false;
    }
    return true;
}
string alabu(v99 i,v99 j,v99 k,v99 l)
{
    string p;p+=char(i+'0');
    p+=char(j+'0');
    p+=char(k+'0');
    ...
    rep0(i,0,10)
    {
        rep0(j,0,10)
        {
            rep0(k,0,10)
            {
                rep0(l,0,10)
                {
                    if(ok(i,j,k,l))mpp[alabu(i,j,k,l)]++;
                }
            }
        }
    }
