...
void solve()
{
    ...
    rep1(i,1,n)
    {
        ...
        sort(sob(vp[i]));
        v99 l=vp[i][0],r=vp[i].back();
        v99 s1=sumr+abs(lastr-r)+(r-l);/// 1
        v99 s2=suml+abs(lastl-r)+(r-l);/// 2

        v99 s3=suml+abs(lastl-l)+(r-l);/// 3
        v99 s4=sumr+abs(lastr-l)+(r-l);/// 4
        suml=min(s1,s2);
        sumr=min(s3,s4);

        lastl=l;lastr=r;
    }
    print(min(suml+abs(lastl),sumr+abs(lastr)));
}
