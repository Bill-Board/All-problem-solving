...
v99 fre[mxi],visi[mxi];
void kori(v99 a[],v99 n,v99 m)
{
    //aout1(fre,11);
    rep0(i,0,n)
    {
        ...
        {
            visi[a[i]]=1;
            v99 kot=fre[a[i]],piche=fre[a[i]-1],samne=fre[a[i]+1];
            if(a[i]==1)
            {
                if(kot>1)fre[a[i]+1]++,fre[a[i]]--;
                //aout1(fre,11);
                continue;
            }
            if(a[i]==m && kot==1){fre[a[i]+1]++;fre[a[i]]--;}
            else if(kot>2)fre[a[i]-1]++,fre[a[i]+1]++,fre[a[i]]-=2;
            else if(kot<=2 && i==0)fre[a[i]-1]++,fre[a[i]]--;
            else if(kot==2 && !piche)fre[a[i]-1]++,fre[a[i]]--;
            else if(kot==2 )fre[a[i]+1]++,fre[a[i]]--;
            else if(kot ==1 && !piche)fre[a[i]-1]++,fre[a[i]]--;
            //aout1(fre,11);
...
v99 solve()
{
    ...
    rep0(i,0,n)cin>>a[i],m=max(m,a[i]),fre[a[i]]++;
    sort(a,a+n);
    //aout0(a,n);
    kori(a,n,m);
    cout<<acha()<<ses;
