...
void solve()
{
    ...
    v99 cr_h=0,nn=(n+1)/2,mm=(m+1)/2;
    rep1(i,1,nn)
    {
        rep1(j,1,mm)
        {
            v99 kot=LLONG_MAX,x=a[i][j],pos=n-i+1,pos2=m-j+1;
            v99 y=a[pos][j],p=a[i][pos2];
            v99 q=a[pos][pos2];
            set<pii>vc;
            vc.insert(mp(i,j));
            vc.insert(mp(pos,j));
            vc.insert(mp(i,pos2));
            vc.insert(mp(pos,pos2));
            for(pii k:vc)
            {
                v99 sum=0;
                for(pii f:vc)
                {
                    v99 dif=abs(a[k.ff][k.ss]-a[f.ff][f.ss]);
                    sum+=dif;
                }
                kot=min(kot,sum);
            }
