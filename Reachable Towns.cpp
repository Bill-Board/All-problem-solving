...
bool sortt(kia &x,kia &y)
{
    return x.a<y.a;
}
v99 n,ans[mxi],visi[mxi],rrr[mxi],sze[mxi];
vector<kia>p;
...
void dfs(v99 ind)
{
    if(visi[ind]==1)return;
    visi[ind]=1;
    for(v99 j=ind+1;j<n;j++)
    {
        if(p[ind].b<p[j].b){dsu(ind,j);dfs(j);}
    }
}
...
void solve()
{
    ...
    rep0(i,0,n)
    {
        ...
        for(auto j:tom[i])ans[p[j].c]=tom[i].size();
    }
    rep1(i,1,n)cout<<ans[i]<<ses;
}
