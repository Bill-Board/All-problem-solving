...
{
    /// code is here->
    v99 n;cin>>n;v99 a[n+2]={},b[n+2]={};
    vector<pii>vp(n);
    rep0(i,0,n)cin>>vp[i].ff;
    rep0(i,0,n)cin>>vp[i].ss;
    sort(sob(vp));
    for(v99 i=n-1;~i;i--)b[i]=b[i+1]+vp[i].ss,a[i+1]=vp[i].ff;
    v99 cr_h=LLONG_MAX;
    rep1(i,0,n)cr_h=min(cr_h,max(a[i],b[i]));
    cout<<cr_h<<ses;
}
